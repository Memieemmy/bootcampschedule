...
export function SessionCard({ sess, si, dateKey, sessions, isEditing, onStartEdit, onStopEdit, onSetSess, onRmSess, getModuleColor, modules }) {
  const isHol = sess.module?.toLowerCase().includes('holiday');
  const pillCol = getModuleColor(sess.module || "");

  return (
    // 👇 ในอนาคต: ห่อด้วย <Draggable> ตรงนี้
    <div
      className="hov-lift"
      style={{
        ...
        position:"relative", cursor: isEditing ? "default" : "pointer",
  ...
  return (
    <>
      <div style={{
        ...
        display:"inline-block", background: sess.module ? pillCol.bg : "transparent",
      ...
      <div style={{
        fontSize:11.5, color: sess.topic ? "#111827" : "#C0C0C8",
        fontWeight: sess.topic ? 500 : 400, lineHeight:1.5, marginBottom:4, minHeight:18
      }}>
      ...
      {sess.module && !isHol && (
        <>
          <div style={{
            marginTop:2, paddingTop:4, borderTop:"1px dotted #E5E7EB",
            fontSize:10, color:"#374151", display:"flex", alignItems:"center", gap:6
          }}>
  ...
  return (
    <div onClick={e => e.stopPropagation()}>
      <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:5 }}>
        <span style={{ fontSize:8, fontWeight:700, color:"#6366F1", textTransform:"uppercase", letterSpacing:.8 }}>
          S{si + 1} · Edit
        </span>
      ...
      <select
        value={sess.module}
        onChange={e => onSetSess(dateKey, si, "module", e.target.value)}
      ...
      >
        ...
        <optgroup label="── Modules ──">
          {Object.keys(modules).filter(m => !m.toLowerCase().includes('holiday')).map(m => (
      ...
      {sess.module && !isHol && (
        <select
          value={sess.topic}
          onChange={e => onSetSess(dateKey, si, "topic", e.target.value)}
        ...
        >
          ...
          {(modules[sess.module] || []).map((t, ti) => (
            <option key={ti} value={t}>{t}</option>
          ))}
        </select>
      )}
      ...
      {sess.module && !isHol && (
        <div style={{ display:"flex", alignItems:"center", gap:3, marginBottom:4 }}>
          ...
          <input type="time" value={sess.start_time}
            onChange={e => onSetSess(dateKey, si, "start_time", e.target.value)}
          ...
          <input type="time" value={sess.end_time}
            onChange={e => onSetSess(dateKey, si, "end_time", e.target.value)}
            style={{ flex:1, padding:"2px 5px", borderRadius:5, border:"1px solid #E5E7EB", fontSize:9, background:"#FAFAFA", outline:"none", color:"#374151", minWidth:0 }}
          />
        </div>
      )}
      ...
      {sess.module && (
        <textarea
          value={sess.note || ""}
          onChange={e => onSetSess(dateKey, si, "note", e.target.value)}
      ...
      {sess.id && sessions.length > 1 && (
        <button
          onClick={() => { onRmSess(dateKey, si); onStopEdit(); }}
